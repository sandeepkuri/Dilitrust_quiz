...
def google_login(driver):
    ...
    driver.find_element_by_xpath(Elements.google_elements['email_address']).send_keys("sandeepkuri001@gmail.com")
    next_button = driver.find_element_by_xpath(Elements.google_elements['b_next'])
    next_button.click()
    driver.find_element_by_xpath(Elements.google_elements['password']).send_keys("sandy100mahla001")
    try:
        next_button.click()
